...
function CustomerTable({ snapshotId }: { snapshotId: Id<"ageSnapshots"> | undefined }) {
  ...
  const effectiveSortKey = sortKey ?? (filter === "risk_120" ? "days120" : null);
  ...
  const sortedRows = [...filteredRows].sort((a, b) => {
    ...
    const valA = (a as any)[effectiveSortKey];
    const valB = (b as any)[effectiveSortKey];
    ...
    if (strA < strB) return effectiveSortDirection === "asc" ? -1 : 1;
    if (strA > strB) return effectiveSortDirection === "asc" ? 1 : -1;
    return 0;
  });
