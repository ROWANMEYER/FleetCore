...
import { PackageOpen, Plus, X, CheckCircle, Loader2} from"lucide-react";
...
function DailyPlannerInputForm() {
 ...
 {/* Sticky Header */}
 <div className="sticky top-0 z-10 bg-[var(--card-bg)]/90 /70 backdrop-blur-lg -mx-8 px-8 pt-8 pb-4 border-b border-[var(--card-border)] shadow-sm mb-8 -mt-8">
 <h1 className="text-3xl font-bold tracking-tight text-[var(--foreground)]">
 {mode ==="edit" ?"Edit Route" :"New Route"}
 </h1>
 <p className="text-sm text-[var(--foreground)] mt-1">Create and manage your fleet routes efficiently</p>
 </div>
...
`}>
 ...
 <div className="text-sm text-[var(--foreground)] bg-[var(--card-bg)] dark:backdrop-blur px-4 py-2 rounded-lg border border-[var(--card-border)] shadow-sm">
 ...
 {totals.quantityDisplay} ΓÇó <span className="font-semibold text-[var(--foreground)]">{formatZAR(totals.revenue)}</span>
 </span>
)}
 </div>
 </div>
 ...
 <div key={load.id} className="p-4 border border-[var(--card-border)] rounded-lg bg-[var(--card-bg)] dark:backdrop-blur-md hover:bg-[var(--card-bg)] transition-all duration-200 relative group shadow-sm">
 <div className="flex justify-between items-start">
 <div className="space-y-1 flex-1">
 <div className="font-medium text-[var(--foreground)] flex items-center gap-2">
 <span className="inline-flex items-center justify-center w-5 h-5 rounded bg-[var(--card-bg)] text-[var(--foreground)] text-xs font-bold">{load.sequence}</span>
 {load.clientName}
 </div>
 <div className="text-sm text-[var(--foreground)]">
 {load.fromLocations.join(",")} ΓåÆ {load.toLocations.join(",")}
 </div>
 </div>
 <div className="text-right space-y-1 pl-4">
 <div className="font-medium text-[var(--foreground)]">
 {load.quantity} <span className="text-[var(--nav-text-color)] text-sm">{unitMap[load.quantityType] || load.quantityType}</span>
 </div>
 <div className="text-sm text-[var(--foreground)]">
 {formatZAR(parseFloat(load.rate) || 0)} <span className="text-xs text-[var(--nav-text-color)]">/{load.rateType ==="flat" ?"flat" :"unit"}</span>
 </div>
 <div className="font-medium text-[var(--foreground)] text-sm pt-2 border-t border-[var(--card-border)] mt-2">
 ...
 <div className="mt-2 pt-2 border-t border-dashed border-[var(--card-border)] space-y-0.5">
 <div className="flex justify-between text-xs text-[var(--nav-text-color)]">
 <span>Sub cost:</span>
 <span className="text-orange-500">{formatZAR(calculateLoadAmount(parseFloat(load.quantity) || 0, parseFloat(load.subcontractorRate) || 0, load.subcontractorRateType ||"per_unit"))}</span>
 </div>
 ...
 className="p-2 text-[var(--nav-text-color)] hover:text-red-600 rounded hover:bg-red-100/50 transition-colors"
 title="Remove Load"
 >
 ...
 {/* Add Load Form */}
 <div className={`border border-[var(--card-border)] rounded-xl p-6 bg-[var(--card-bg)] dark:backdrop-blur-xl shadow-lg transition-all duration-300 ${!isEditable ?"opacity-50 pointer-events-none" :""}`}>
 <h4 className="font-semibold text-[var(--foreground)] mb-4 flex items-center gap-2">
 <Plus className="w-5 h-5" /> Add a New Load
 </h4>
 ...
 className="w-full h-10 px-3 rounded border border-[var(--card-border)] bg-[var(--card-bg)]/60 dark:backdrop-blur-sm shadow-sm focus:border-[#06B6D4] focus:ring-2 focus:ring-[#06B6D4]/30 focus:outline-none text-sm transition-colors text-[var(--foreground)]"
 ...
 className="flex-1 h-10 px-3 rounded border border-[var(--card-border)] bg-[var(--card-bg)]/60 dark:backdrop-blur-sm shadow-sm focus:border-[#06B6D4] focus:ring-2 focus:ring-[#06B6D4]/30 focus:outline-none text-sm transition-colors text-[var(--foreground)]"
 placeholder="Pickup Location"
 />
 {draftLoad.fromLocations.length > 1 && (
 <button
 onClick={() => removeLocationField("from", i)}
 className="px-3 text-[var(--nav-text-color)] hover:text-red-600 hover:bg-red-50/80 dark:hover:bg-red-500/10 rounded-lg transition-colors"
 >
 <X className="w-4 h-4" />
 </button>
)}
 ...
 className="text-xs text-[var(--nav-text-color)] hover:text-[var(--foreground)] dark:hover:text-white font-medium hover:bg-[var(--card-bg)] px-3 py-1.5 rounded transition-colors"
 >
 + Add Pickup
 </button>
 </div>
 ...
 className="h-10 px-3 rounded border border-[var(--card-border)] bg-[var(--card-bg)]/60 dark:backdrop-blur-sm shadow-sm focus:border-[#06B6D4] focus:ring-2 focus:ring-[#06B6D4]/30 focus:outline-none text-sm transition-colors appearance-none cursor-pointer text-[var(--foreground)]"
 >
 {unitOptions.map(opt => (
 ...
 <span className="text-sm font-semibold text-[var(--foreground)]">Subcontractor Rate</span>
 </div>
 <div className="grid grid-cols-2 gap-4">
 <div>
 <label className="block text-xs font-medium text-orange-700 dark:text-orange-400 mb-1.5">Sub Rate (what you pay sub)</label>
 <input
 type="number"
 value={draftLoad.subcontractorRate}
 onChange={(e) => setDraftLoad({ ...draftLoad, subcontractorRate: e.target.value})}
 className="w-full h-10 px-3 rounded border border-orange-300 dark:border-orange-700/50 bg-[var(--card-bg)]/60 shadow-sm focus:border-orange-500 focus:ring-2 focus:ring-orange-500/30 focus:outline-none text-sm transition-colors text-[var(--foreground)]"
 placeholder="0.00"
 />
 </div>
 <div>
 <label className="block text-xs font-medium text-orange-700 dark:text-orange-400 mb-1.5">Sub Rate Type</label>
 <select
 value={draftLoad.subcontractorRateType}
 onChange={(e) => setDraftLoad({ ...draftLoad, subcontractorRateType: e.target.value})}
 className="w-full h-10 px-3 rounded border border-orange-300 dark:border-orange-700/50 bg-[var(--card-bg)]/60 shadow-sm focus:border-orange-500 focus:ring-2 focus:ring-orange-500/30 focus:outline-none text-sm transition-colors appearance-none cursor-pointer text-[var(--foreground)]"
 >
 ...
 className="w-full bg-[var(--card-bg)] hover:bg-[var(--card-bg)] text-white px-6 py-2.5 rounded text-sm font-medium shadow-sm hover:shadow-md transition-all duration-200 flex justify-center items-center gap-2"
 >
 <span>Add Load</span>
 </button>
 </div>
 ...
 className={`
 bg-[var(--card-bg)] hover:bg-black text-white px-8 py-2.5 rounded text-sm font-medium shadow-md hover:shadow-lg transition-all duration-200 /80 dark:backdrop-blur-sm dark:hover:bg-[var(--card-bg)]/80
