    ...
    <div className="h-full min-h-0 flex flex-col relative overflow-hidden">
      ...
      {editRouteId && (
        <div className="absolute inset-0 z-50 flex justify-end bg-black/40 backdrop-blur-md animate-in fade-in duration-200">
          <div className="w-[600px] h-full bg-white/90 shadow-2xl border-l border-white/40 flex flex-col animate-in slide-in-from-right duration-300 backdrop-blur-lg">
      ...
      <div className="lg:hidden flex-shrink-0">
        <div className="border-b px-8 pt-6 bg-white/0 backdrop-blur-lg border-white/10">
          <h2 className="text-lg font-semibold mb-4 text-gray-900">Daily Planner</h2>
      ...
      <div className="lg:hidden p-8 flex-1 overflow-y-auto scrollbar-hidden bg-white/0 backdrop-blur-sm">
        {children}
      </div>
      ...
      <div className="hidden lg:flex flex-1 overflow-hidden w-full relative min-h-0">
        ...
        {/* Left Pane — Input */}
        <div 
          className="h-full overflow-y-auto overscroll-y-contain scrollbar-hidden min-h-0 min-w-0 bg-white/0 backdrop-blur-lg border-r border-white/10" 
          style={{ width: `${leftWidth}%` }} 
        > 
          <div className="min-h-full p-8 max-w-none w-full bg-white/10 backdrop-blur-xl border border-white/10 rounded-xl shadow-sm"> 
            <InputPage />
          </div>
        </div> 
        ...
        {/* Resizer */} 
        <div 
          className="w-1.5 cursor-col-resize bg-black/20 hover:bg-black/30 transition-colors duration-200" 
          onMouseDown={startResize}
        /> 
      
        {/* Right Pane — Sheets */} 
        <div 
          className="h-full overflow-hidden min-h-0 min-w-0 flex-1 bg-white/0 backdrop-blur-lg border-l border-white/10" 
        > 
          <div className="h-full overflow-hidden p-4 w-full bg-white/10 backdrop-blur-xl border border-white/10 rounded-xl shadow-sm"> 
            <SheetsPage />
          </div>
        </div> 
      </div>
