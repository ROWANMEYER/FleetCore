...
export function EmptyState({ icon = "empty", title, description, action }: EmptyStateProps) {
  return (
    <div className="flex flex-col items-center justify-center py-16 px-4 text-center">
      ...
      {description && (
        <p className="text-xs text-[var(--nav-text-color)] max-w-xs">{description}</p>
      )}
      {action && (
        <button
          onClick={action.onClick}
          className="mt-4 px-4 py-2 text-xs font-medium bg-gradient-to-br from-[#06B6D4] to-[#0891B2] text-white rounded-lg hover:opacity-90 transition-opacity"
        >
          {action.label}
        </button>
      )}
